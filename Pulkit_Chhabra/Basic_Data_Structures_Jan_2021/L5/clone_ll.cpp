...
public:
	...
	node* SinglyReverseK(node *cur_head, int k)
	{
		//reverse 1st min(k,len) elements
		if (!cur_head)
			return NULL;
		...
		cur_head->next = SinglyReverseK(cur, k);
...
node *cloneLL(node *head)
{
	...
	while (cur->next)
	{
		node *newNext = new node(cur->next->data);
...
/*
node *cloneLL_with_Random(node *head)
{
	node *newHead = new node(head->data);
...
int32_t main()
{
	...
	l.pb(1), l.pb(2), l.pb(3);

	node *cloned = cloneLL(l.head);
